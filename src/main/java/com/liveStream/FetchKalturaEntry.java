...
public class FetchKalturaEntry extends HttpServlet {
	...
	private final static Logger LOGGER = Logger
			.getLogger(FetchKalturaEntry.class.getName());
	...
	protected void doGet(HttpServletRequest request,
		...
		if (context.getAttribute("videoduration") != null) {
			time = Integer.parseInt(context.getAttribute("videoduration")
					.toString());
		}
		if (context.getAttribute("taggedVideo") != null) {
			entryId = context.getAttribute("taggedVideo").toString();
		}/*
		 * else{ context.setAttribute("taggedVideo","1_t2ssbfzu"); entryId =
		 * context.getAttribute("taggedVideo").toString(); }
		 */
